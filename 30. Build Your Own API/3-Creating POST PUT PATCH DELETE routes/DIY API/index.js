...
app.get("/jokes/:id", (req, res) => {
  ...
  console.log(id);
  const findJokes = jokes.find((joke) => id === joke.id);
...
app.get("/filter", (req, res) => {
  const typeJokes = req.query.type;
  console.log(typeJokes);
  const filteredJokesType = jokes.filter((joke) => typeJokes === joke.jokeType);

  if (filteredJokesType.length > 0) {
    res.json(filteredJokesType);
  } else {
    res.status(404).json({ error: `Joke with type ${typeJokes} not found.` });
  }
});
...
app.post("/jokes", (req, res) => {
  ...
  if (!req.body || !req.body.text || !req.body.type) {
    res.status(401).json({ error: `Joke text and type are required.` });
  } else {
...
app.put("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const jokesIndex = jokes.findIndex((joke) => id === joke.id);
  ...
  if (jokesIndex !== -1) {
    ...
    if (!req.body || !req.body.text || !req.body.type) {
      ...
      // Update Jokes on index
      jokes[jokesIndex].jokeText = req.body.text;
      jokes[jokesIndex].jokeType = req.body.type;

      res.json(jokes[jokesIndex]);
    }
  } else {
    // return error. Ff index Jokes ID not found
    res.status(404).json({ error: `Joke with ID ${id} not found.` });
  }
});
...
app.delete("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const findIndex = jokes.findIndex((joke) => id === joke.id);

  if (findIndex !== -1) {
    jokes.splice(findIndex, 1);
    res.status(200).json({ success: `Joke with ID ${id} has been deleted` });
  } else {
    res.status(404).json({ error: `Joke with ID ${id} not found.` });
  }
});
...
app.delete("/all", (req, res) => {
  ...
  if (userKey === masterKey) {
    ...
    res.status(200).json({ success: `All jokes databases has been deleted.` });
  } else {
